...
def check_authorization_files():
    ...
    for auth_file in auth_dir.glob("*.json"):
        # Skip audit logs or other non-authorization JSON files
        if auth_file.name.lower() == "audit_log.json":
            continue
        try:
            ...
            if not isinstance(auth_data, dict):
                log(f"Skipping non-authorization JSON file: {auth_file.name}", Colors.YELLOW)
                continue
...
def check_disk_space():
    ...
    try:
        ...
        usage_percent = int((usage.used / usage.total) * 100)

        if usage_percent > 90:
            log(f"⚠️  Disk usage critical: {usage_percent}%", Colors.RED)
        elif usage_percent > 80:
            log(f"⚠️  Disk usage high: {usage_percent}%", Colors.YELLOW)
        else:
            log(f"✓ Disk usage OK: {usage_percent}%", Colors.GREEN)
    except Exception as e:
...
def check_python_dependencies():
    ...
    try:
        # Prefer pip-audit if available (via pipx or pip)
        if shutil.which("pip-audit") is None:
            log("ℹ️  Install pip-audit for vulnerability scanning: pip install pip-audit", Colors.BLUE)
            log("   (pipx install pip-audit is also supported)", Colors.BLUE)
            return
...
def check_tool_versions():
    ...
    for tool_name, cmd in tools.items():
        ...
        if shutil.which(binary) is None:
            log(f"  {tool_name}: Not found in PATH", Colors.YELLOW)
            continue
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                log(f"  {tool_name}: Unable to determine version (exit {result.returncode})", Colors.YELLOW)
        except Exception as e:
            log(f"  {tool_name}: Error checking version: {e}", Colors.YELLOW)
