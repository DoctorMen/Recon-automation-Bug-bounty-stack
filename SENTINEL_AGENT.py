...
try:
    ...
    print("⚠️  Neural Network Brain not found - using standard scoring")
...
class SentinelAgent:
    ...
    def __init__(self, target, tier='basic', output_dir='./assessments'):
        ...
        if NEURAL_BRAIN_ENABLED:
            ...
            print(f"\n🧠 SENTINEL AGENT INITIALIZED with Neural Brain")
            print(f"   Target: {target}")
            print(f"   Tier: {tier}")
        else:
            self.neural_enabled = False
            print(f"\n🔍 SENTINEL AGENT INITIALIZED (Standard Mode)")
            print(f"   Target: {target}")
            print(f"   Tier: {tier}")
    ...
    def calculate_risk_score(self):
        """Calculate overall risk score (0-100) with neural intelligence"""
        score = 0
        ...
        # Neural Network Brain enhancement
        if self.neural_enabled and self.findings:
            # Create asset representation for neural scoring
            asset = {
                'name': self.target,
                'type': 'domain',
                'severity_counts': self.severity,
                'total_findings': len(self.findings),
                'has_critical': self.severity['critical'] > 0,
                'has_high': self.severity['high'] > 0
            }
            
            # Get neural score
            neural_score = self.prioritizer.score(asset)
            neural_boost = int(neural_score * 20)  # Convert to 0-20 point boost
            
            print(f"      🧠 Neural Score: {neural_score:.3f} (boost: +{neural_boost})")
            score += neural_boost
