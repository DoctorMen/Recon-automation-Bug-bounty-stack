...
RATE_LIMIT = int(os.getenv("HTTPX_RATE_LIMIT", "150"))  # 150 req/sec (3x faster)
TIMEOUT = int(os.getenv("HTTPX_TIMEOUT", "10"))
THREADS = int(os.getenv("HTTPX_THREADS", "100"))  # 100 concurrent connections
RETRIES = int(os.getenv("HTTPX_RETRIES", "2"))
MAX_REDIRECTS = int(os.getenv("HTTPX_MAX_REDIRECTS", "10"))
...
def main():
    ...
    # Run httpx - OPTIMIZED FOR SPEED & DEPTH
    log(f"Running httpx (rate-limit: {RATE_LIMIT}/s, threads: {THREADS}, max-redirects: {MAX_REDIRECTS})...")
    try:
        result = subprocess.run(
            [
                ...
                "-retries", str(RETRIES), "-follow-redirects", 
                "-max-redirects", str(MAX_REDIRECTS),
                "-content-length", "-method", "-websocket", 
                "-pipeline", "-http2", "-no-fallback-scheme",
                "-o", str(HTTP_TEMP)
            ],
            capture_output=True,
            text=True,
